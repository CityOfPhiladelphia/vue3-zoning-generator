...
console.log('router 1:', router, 'route:', { ...route });
...
const addr_search = (address) => {
        ...
        const requestUrl = `https://api.phila.gov/ais/v1/search/${address}?gatekeeperKey=${gatekeeperKey}`;
...
onMounted(() => {
    console.log('router 2:', router, 'route["params"]:', route['params']);
    var overlayslist = [];
    var overlayDict = {};
    $(".HideUnhide").hide();
    $("#PrintBtn").hide();
    ...
    //     });
    // }
    map = L.map('map', {zoomControl: false}).setView([40.0, -75.14], 12);
